  ...
  module SortableColumns
    ...
    module InstanceMethods
      ...
      def sortable_column(title, options = {}) #:doc:
        ...
        # Parse sort param.
        sort = safe_params[conf[:sort_param]] || conf[:default_sort_value]
        ...
        if pp[:column] == o[:column].to_s
          if o[:route_proxy]
            url = o[:route_proxy].send(:url_for, safe_params.merge(conf[:sort_param] => [('-' if pp[:direction] == :asc), o[:column]].join, conf[:page_param] => 1))
          else
            url = url_for(safe_params.merge(conf[:sort_param] => [('-' if pp[:direction] == :asc), o[:column]].join, conf[:page_param] => 1))
          end
          ...
          # Not sorted.
          if o[:route_proxy]
            url = o[:route_proxy].send(:url_for, safe_params.merge(conf[:sort_param] => [('-' if o[:direction] != :asc), o[:column]].join, conf[:page_param] => 1))
          else
            url = url_for(safe_params.merge(conf[:sort_param] => [('-' if o[:direction] != :asc), o[:column]].join, conf[:page_param] => 1))
          end
